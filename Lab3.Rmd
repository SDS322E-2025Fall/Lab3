...
```{r}
# Copy data and convert to tibble()
...
This dataset contains information about Starwars characters: name, height, mass, hair_color, skin_color, eye_color, birth_year, sex, gender, homeworld, species, films, vehicles, starships. Look at the help file (`?starwars`) to see what **units** these variables are measured in.

The goal of the lab is to investigate some characteristics of the Starwars characters using `dplyr` data transformation functions.
...
```{r}
# your code goes below (make sure to edit this comment)

```

**Write your answer here in bold.**

------------------------------------------------------------------------

### Question 2: (3 pts)

Using `dplyr` core functions, find out how many characters' names there are per species. Which species contain the two largest numbers of character names?
...
Using `dplyr` core functions and `ggplot`, compare the distributions of BMI between the two species found in the previous question using boxplots and simultaneously showing the individual data points with `geom_point()`. Which of the two species seem to have the highest BMI on average? 
...
Choose one of the research questions proposed by your group members in the Pre-Lab Quiz. Answer your research questions using at least 2 of the `dplyr` core functions and with a `ggplot` visualization. Include a headline to your visualization and interpret what you see!
